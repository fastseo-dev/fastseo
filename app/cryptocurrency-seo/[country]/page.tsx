...
import { cryptoCountryLocations, getCryptoCountryData } from "@/data/crypto-country-locations";
...
export async function generateStaticParams() {
  return cryptoCountryLocations.map((l) => ({ country: l.countrySlug }));
}

export async function generateMetadata({ params }: { params: Promise<{ country: string }> }): Promise<Metadata> {
  const { country } = await params;
  const data = getCryptoCountryData(country);
  if (!data) return {};
...
export default async function CryptoCountryPage({ params }: { params: Promise<{ country: string }> }) {
  const { country } = await params;
  const data = getCryptoCountryData(country);
  if (!data) notFound();
  const config = nicheConfigs[NICHE];
  const otherCountries = cryptoCountryLocations.filter((l) => l.countrySlug !== country);
