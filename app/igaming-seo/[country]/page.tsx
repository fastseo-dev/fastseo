...
import { igamingCountryLocations, getIgamingCountryData } from "@/data/igaming-country-locations";
import { nicheConfigs } from "@/data/niche-country-types";

export const dynamicParams = false;

const BASE = "https://www.fastseosolutions.com";
const NICHE = "igaming-seo";

export async function generateStaticParams() {
  return igamingCountryLocations.map((l) => ({ country: l.countrySlug }));
}

export async function generateMetadata({ params }: { params: Promise<{ country: string }> }): Promise<Metadata> {
  const { country } = await params;
  const data = getIgamingCountryData(country);
  if (!data) return {};
...
export default async function IgamingCountryPage({ params }: { params: Promise<{ country: string }> }) {
  const { country } = await params;
  const data = getIgamingCountryData(country);
  if (!data) notFound();
  const config = nicheConfigs[NICHE];
  const otherCountries = igamingCountryLocations.filter((l) => l.countrySlug !== country);
